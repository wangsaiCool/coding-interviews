...
public class PrintMatrixClockWisely {
	/**
	 * 题目：输入一个矩阵，按照从外向里以顺时针的顺序打印出每一个数字。 思路： 循环打印: 1：先打印一行（第1行肯定会打印）
	 * 2：再打印当前矩阵的最后一列 3：再倒序打印当前矩阵的最后一行 4：再倒序打印当前矩阵的第一列 起始坐标的规律：
	 * (0,0),(1,1),(2,2)...(startX,startY),起始坐标的两个坐标值相等。 并且 startX<=
	 * (rows-1)/2,startY<=(columns-1)/2 当前矩阵，第1行 坐标 (start,columns-1-start) =>
	 * (start,endX) 当前矩阵，最后1列 坐标 (start+1,rows-1-start) => (start+1,endY)
	 * 当前矩阵，最后1行 坐标 (start,columns-1-start+1) => (start,endX-1) 当前矩阵，第1行 坐标
